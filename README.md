...
- `r3akt-tak-connector`: dedicated Rust TAK connector crate and service
  ...
  service slice still owns live TAK reconnect validation and the internal
...
live TAK reconnect validation, inbound TAK behavior where RCH needs it,
live multi-recipient fanout/golden-vector validation for mission side effects,
...
| --- | --- | --- | --- | --- |
| LXMF wire/runtime | LXMF-rs | Reuse `lxmf-core` for wire-message byte helpers; wrap live transport via `LxmfRsAdapter` and `MessageBus`; added a `reticulumd` RPC adapter for R3AKT envelope send/poll plus an opt-in Rust-server live direct-receipt validation hook | Real Rust LXMF/RNS crates exist; avoid fake transport | Keep live two-daemon receipt validation in the release gate; add broader real-network fanout validation before default-branch switch |
...
| TAK connector service | RCH PyTAK integration | Added `r3akt-tak-connector` with Python-compatible CoT location/chat XML builders, PyTAK-shaped TAK hello/pong keepalive payloads, bounded outbound queue/backpressure behavior, clear TCP/UDP CoT senders, TLS CoT sender support for `ssl://`/`tls://` with CA/client-cert config, PKCS#12/PFX `tls_client_password` support and explicit unsupported encrypted-PEM failure, structured inbound CoT parsing with Python `ReceiveWorker` raw-fallback behavior, an inbound polling service boundary that records parsed/raw receive results and receiver errors, bounded TCP/UDP/TLS CoT socket receivers, golden-shape tests, local TCP/UDP/TLS loopback tests, local reconnect-after-close coverage, env-gated live TAK keepalive, reconnect push, and inbound receive smokes, an explicit start/stop service lifecycle/status object, Python-compatible TAK TLS config loading with redacted runtime diagnostics covered by regression tests, outbound and inbound retry workers with Python-style exponential send-failure/receive-failure backoff, and the outbound worker wired into `r3akt-rch-server` for configured chat/location CoT dispatch plus TAK ping/keepalive scheduling | TAK server connectivity is its own lifecycle and must remain independently deployable; Rust cannot depend on Python/PyTAK for the final backend switch | Add live TAK bidirectional workflow validation against a real TAK server profile |
...
7. Complete the dedicated `r3akt-tak-connector` service. The crate now owns the
   ...
   server-side close in local loopback coverage. Opt-in live TAK keepalive and
   ...
   remaining service work is live TAK bidirectional workflow validation against a
   real TAK server profile. The Rust server must not embed Python or
   own the final TAK socket lifecycle directly.
...
Not proven by local-only verification yet: live TAK bidirectional validation
against a real TAK server profile and live multi-recipient LXMF fanout
validation against real peers.
