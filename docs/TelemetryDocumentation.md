...
- `FIELD_TELEMETRY_STREAM` is a plain list; each entry is `[peer_hash_bytes, unix_timestamp, packed_payload, appearance?]` assembled in `handle_command()` when RCH serves a telemetry request (`reticulum_telemetry_hub/lxmf_telemetry/telemetry_controller.py:231`). Sideband accepts an optional appearance element; RCH sends `["person", b"\x00\x00\x00", b"\xff\xff\xff"]`-style payloads where the list is `[icon_name, background_rgb_bytes, foreground_rgb_bytes]`. When marker metadata includes a symbol, the icon name comes from the marker symbol registry (MDI entries such as `friendly` -> `rectangle`) and the background color is derived from the symbol key.
...
- Telemetry requests already use machine-readable timestamps (`unix_timestamp`) and never depend on locale-formatted date strings.
- When a telemetry-bearing LXMF message includes the optional `RTHDelivery` envelope, RCH validates the same delivery headers, TTL budget, UTC skew, and canonical `TopicID` rules used by the broader southbound message-delivery path.
...
- **Topic filtering** - Telemetry `TopicID` filters are normalized through the shared canonical topic-ID helper before subscriber checks and database filtering, so topic-scoped telemetry requests follow the same routing identity as chat/topic southbound traffic.
