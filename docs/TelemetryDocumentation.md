...
- `FIELD_TELEMETRY_STREAM` is a plain list; each entry is `[peer_hash_bytes, unix_timestamp, packed_payload, appearance?]` assembled in `handle_command()` when RCH serves a telemetry request (`reticulum_telemetry_hub/lxmf_telemetry/telemetry_controller.py:231`). Sideband accepts an optional appearance element; RCH sends `{4: ["bird", b"\x00\x00\x00", b"\xff\xff\xff"]}` where the list is `[mdi_icon_name, background_rgb_bytes, foreground_rgb_bytes]`.
...
- The reply is placed in `FIELD_TELEMETRY_STREAM` as a plain list of `[peer_hash_bytes, unix_timestamp, packed_payload, appearance]` entries (`reticulum_telemetry_hub/lxmf_telemetry/telemetry_controller.py:293`). Sideband consumes that stream directly, updating the live track without any RCH-specific UI logic.
