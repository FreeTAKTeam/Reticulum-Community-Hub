...
| Release packages carry traceable version metadata | `.github/workflows/rust-release.yml`, `scripts/build-rust-release-package.ps1`, `release-manifest.json` inside the server archive | Server archive names now include the resolved release version from the GitHub release tag, pushed tag, or manual workflow input. `release-manifest.json` records the same release version plus `GITHUB_REF_NAME` and `GITHUB_SHA`. A local Windows server-only package smoke on 2026-05-30 produced `rch-rust-full-windows-x64-v3.0.0-preview.0-local.zip` with `release_version=v3.0.0-preview.0`, `git_ref=v3.0.0-preview.0`, and the supplied Git SHA in the manifest. | Passed locally; pending CI packaging run |
