...
@pytest.mark.parametrize("backend", ["python", "rust"])
def test_subscriber_command_retrieve_patch_delete_round_trip(tmp_path, backend: str):
    if RNS.Reticulum.get_instance() is None:
        RNS.Reticulum()
    ...
    subscriber = api.create_subscriber(
        Subscriber(destination="peer-1", topic_id=topic.topic_id)
    )
    manager, server_dest = make_command_manager(api)
    client_dest = RNS.Destination(
        RNS.Identity(), RNS.Destination.OUT, RNS.Destination.SINGLE, "lxmf", "delivery"
    ...
    assert retrieve_reply is not None
    assert '"Destination": "peer-1"' in retrieve_reply.content_as_string()
    ...
    assert patch_reply is not None
    assert "Subscriber updated" in patch_reply.content_as_string()
    ...
    assert delete_reply is not None
    assert "Subscriber deleted" in delete_reply.content_as_string()
    with pytest.raises(KeyError):
...
def test_delivery_callback_skips_help_for_unjoined_telemetry_only_messages():
    if RNS.Reticulum.get_instance() is None:
        RNS.Reticulum()
...
def test_delivery_callback_skips_extra_help_for_unjoined_command_messages():
    if RNS.Reticulum.get_instance() is None:
        RNS.Reticulum()
    ...
    def command_reply(commands, message):
        return [LXMF.LXMessage(sender, hub.my_lxmf_dest, "command-ok")]
    ...
    hub.command_manager = type(
        ...
        (),
        {
            "handle_commands": lambda self, commands, message: command_reply(commands, message),
            "_handle_help": lambda self, message: build_help(message),
        },
    )()
    hub.send_message = MethodType(ReticulumTelemetryHub.send_message, hub)
    ...
    assert len(router_messages) == 1
    assert router_messages[0].content_as_string() == "command-ok"


def test_delivery_callback_ignores_passive_unknown_numeric_command_payloads():
    if RNS.Reticulum.get_instance() is None:
        RNS.Reticulum()
...
def test_delivery_callback_preserves_unknown_numeric_command_replies():
    ...
    incoming = LXMF.LXMessage(
        hub.my_lxmf_dest,
        sender,
        fields={LXMF.FIELD_COMMANDS: [{PLUGIN_COMMAND: "PassiveHeartbeat"}]},
        desired_method=LXMF.LXMessage.DIRECT,
    )
    incoming.signature_validated = True
