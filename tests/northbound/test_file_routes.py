...
from tests.test_rth_api import _attachment_test_path
...
def _build_client(
    ...
    *,
    backend: str = "python",
) -> Tuple[TestClient, ReticulumTelemetryHubAPI | RustTopicSubscriberApi]:
    ...
    api = (
        ...
        if backend == "rust"
        else ReticulumTelemetryHubAPI(config_manager=make_config_manager(tmp_path))
    )
    app = create_app(
        api=api,
...
def test_file_routes_return_empty_lists(tmp_path: Path, backend: str) -> None:
...
@pytest.mark.parametrize("backend", ["python", "rust"])
def test_file_routes_return_metadata(tmp_path: Path, backend: str) -> None:
    """Verify file and image metadata routes return stored attachments."""

    client, api = _build_client(tmp_path, backend=backend)
    headers = {"X-API-Key": "secret"}
    file_path = _attachment_test_path(api, tmp_path, "file", "note.txt")
    file_path.write_text("hello")
    file_record = api.store_file(file_path, media_type="text/plain")
    image_path = _attachment_test_path(api, tmp_path, "image", "photo.jpg")
...
@pytest.mark.parametrize("backend", ["python", "rust"])
def test_file_and_image_routes_paginate_metadata(tmp_path: Path, backend: str) -> None:
    """Verify file and image list routes return paginated envelopes when requested."""

    client, api = _build_client(tmp_path, backend=backend)
    headers = {"X-API-Key": "secret"}
    for index in range(3):
        file_path = _attachment_test_path(api, tmp_path, "file", f"note-{index}.txt")
        file_path.write_text(f"hello {index}")
        api.store_file(file_path, media_type="text/plain")
    for index in range(2):
        image_path = _attachment_test_path(
            api, tmp_path, "image", f"photo-{index}.jpg"
        )
        image_path.write_bytes(b"img")
...
@pytest.mark.parametrize("backend", ["python", "rust"])
def test_file_routes_patch_topic_association(tmp_path: Path, backend: str) -> None:
    ...
    file_path = _attachment_test_path(api, tmp_path, "file", "topic.txt")
    file_path.write_text("hello")
    image_path = _attachment_test_path(api, tmp_path, "image", "topic.jpg")
    image_path.write_bytes(b"img")
    file_record = api.store_file(file_path, media_type="text/plain")
    image_record = api.store_image(image_path, media_type="image/jpeg")
...
@pytest.mark.parametrize("backend", ["python", "rust"])
def test_file_routes_patch_missing_entries_return_404(tmp_path: Path, backend: str) -> None:
...
@pytest.mark.parametrize("backend", ["python", "rust"])
def test_file_routes_return_raw_bytes(tmp_path: Path, backend: str) -> None:
...
@pytest.mark.parametrize("backend", ["python", "rust"])
def test_file_routes_missing_entries_return_404(tmp_path: Path, backend: str) -> None:
...
@pytest.mark.parametrize("backend", ["python", "rust"])
def test_file_routes_delete_entries(tmp_path: Path, backend: str) -> None:
    """Ensure delete routes remove files/images from metadata and disk."""

    client, api = _build_client(tmp_path, backend=backend)
    headers = {"X-API-Key": "secret"}
    file_path = _attachment_test_path(api, tmp_path, "file", "delete.txt")
    file_path.write_bytes(b"payload")
    file_record = api.store_file(file_path, media_type="text/plain")
    image_path = _attachment_test_path(api, tmp_path, "image", "delete.jpg")
...
@pytest.mark.parametrize("backend", ["python", "rust"])
def test_file_routes_delete_missing_entries_return_404(tmp_path: Path, backend: str) -> None:
...
@pytest.mark.parametrize("backend", ["python", "rust"])
def test_file_routes_require_auth_for_remote_clients(tmp_path: Path, backend: str) -> None:
    client, _ = _build_client(tmp_path, backend=backend)
