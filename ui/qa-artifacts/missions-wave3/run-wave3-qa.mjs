...
const outDir = path.resolve("qa-artifacts", "missions-wave3");
...
try {
  ...
  const seedResult = await page.evaluate(async (apiRoot) => {
    ...
    if (missions.length < 2) {
      ...
      const createPayload = {
        mission_name: `QA Wave3 ${stamp}`,
        mission_status: "MISSION_ACTIVE",
        description: "QA seed mission for Missions Wave 3 manual QA"
      };
  ...
  if (seedResult.ok) {
    ...
    addCheck("seed-wave3", "FAIL", "Mission data seed for workspace checks", seedResult.error || "Unknown seed error");
  }
  ...
  if ((await broadcastBtn.count()) > 0) {
    ...
    addCheck("5.2-prep-wave3", "PASS", "Prepare mission audit sample data", "Broadcast action executed");
  } else {
    addCheck("5.2-prep-wave3", "FAIL", "Prepare mission audit sample data", "Broadcast button not found");
  }
  ...
  if (detailsCount > 0) {
    ...
    addCheck("5.4-wave3", "FAIL", "Audit row action is keyboard/click accessible and reveals details", "No enabled detail buttons");
  }
  ...
  if ((await assetsBtn.count()) > 0) {
    ...
    addCheck("5.3-wave3", "FAIL", "Asset registry list is scrollable and actions are below list", "Assets button not found");
  }
  ...
  if ((await templatesTab.count()) > 0) {
    ...
    addCheck("6.3-wave3", "FAIL", "Checklist tabs update selected state and content", "Templates tab not found");
  }
  ...
  if ((await newButton.count()) > 0) {
    ...
    addCheck("6.1-focus-trap-wave3", "BLOCKED", "Modal focus trap wraps from last to first focusable element", "New button not found");
    addCheck("6.1-escape-wave3", "BLOCKED", "Modal closes on Escape", "New button not found");
  }
  ...
  const responsiveProfiles = [
    { id: "7.1-wave3", width: 1440, height: 900, file: "missions-desktop-1440-wave3.png", label: "Responsive stability check at 1440x900" },
    { id: "7.2-wave3", width: 1280, height: 800, file: "missions-laptop-1280-wave3.png", label: "Responsive stability check at 1280x800" },
    { id: "7.3-wave3", width: 900, height: 1280, file: "missions-tablet-900-wave3.png", label: "Responsive stability check at 900x1280" },
    { id: "7.4-wave3", width: 390, height: 844, file: "missions-mobile-390-wave3.png", label: "Responsive stability check at 390x844" },
    { id: "7.5-wave3", width: 360, height: 800, file: "missions-mobile-360-wave3.png", label: "Responsive stability check at 360x800" }
  ];
  ...
  const resultPath = path.join(outDir, "manual-qa-results-wave3.json");
  await fs.writeFile(
    resultPath,
    JSON.stringify(
      {
  ...
  console.log(`Wrote QA wave3 results: ${resultPath}`);
  console.log(`Checks recorded: ${checks.length}`);
} catch (error) {
  console.error("QA wave3 script failed:", error);
  process.exitCode = 1;
} finally {
  await browser.close();
}
